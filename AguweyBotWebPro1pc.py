# ============================================
# AGUWEYBOT - VERSIÓN CON GENERADOR LOCAL MEJORADO
# ============================================
...
# ============================================
st.set_page_config(
    page_title="AguweyBot - Generador de Imágenes Local",
    page_icon="🎨",
    layout="wide",
...
if "GROQ_API_KEY" not in st.secrets:
    ...
    st.info("Por favor, agrega tu GROQ_API_KEY en Settings → Secrets")
    st.stop()
...
def generar_imagen_local(descripcion: str) -> Tuple[Optional[bytes], Optional[str]]:
    ...
    try:
        ...
        try:
            ...
            # Texto informativo
            info_text = "🎨 Generado por AguweyBot (Modo Local)"
        ...
        try:
            ...
            draw.text((400, 350), "Modo local - AguweyBot", fill=(0, 255, 255), anchor="mm")
            img_bytes = io.BytesIO()
...
def mostrar_logo():
    if os.path.exists(Config.LOGO_PATH):
        ...
        st.sidebar.markdown("""
        # 🤖 AguweyBot
        ### *Asistente con generación local de imágenes*
        """)
...
def main():
    ...
    # Contenido principal
    st.markdown("<h1>🎨 AguweyBot - Generador Local de Imágenes</h1>", unsafe_allow_html=True)
    ...
    if prompt:
        ...
        if descripcion_imagen:
            ...
            # Respuesta de texto normal con Groq
            with st.chat_message("assistant"):
                try:
                    container = st.empty()
                    ...
                    container.markdown(f'<div class="respuesta-aguwey">{response.content}</div>', unsafe_allow_html=True)
                    st.session_state.messages.append({"role": "assistant", "content": response.content})
                    
                    if TTS_AVAILABLE and len(response.content) > 100:
                        audio_bytes = texto_a_audio_unico(response.content)
                        if audio_bytes:
                            st.session_state.audio_actual_bytes = audio_bytes
    ...
    st.markdown(
        ...
        <div class="fixed-footer">
            <strong>CC-SA</strong> Prof. Raymond Rosa Ávila • AguweyBot • 
            <span data-tooltip="Versión con generador local mejorado">🚀 v8.0</span>
        </div>
        """,
        unsafe_allow_html=True
    )
